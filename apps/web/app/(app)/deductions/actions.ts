...
const NOT_ALLOWED = "Hanya pemilik/admin yang dapat mengubah potongan gaji.";
...
const customSchema = z
  .object({
    ...
    // Whole rupiah for a fixed deduction.
    amount: z.coerce.number().int().min(0).default(0),
    // Percentage (e.g. 2.5) for a percentage deduction; stored as basis points.
    ratePercent: z.coerce.number().min(0).max(100).default(0),
    base: z.enum(["gross", "base_salary"]).default("gross"),
  })
  .refine((d) => (d.calc === "fixed" ? d.amount > 0 : d.ratePercent > 0), {
    message: "Masukkan nominal atau persentase potongan.",
  });
...
export async function createCustomDeduction(
  _prev: DeductionState,
  formData: FormData,
...
export async function updateCustomDeduction(
  _prev: DeductionState,
  formData: FormData,
): Promise<DeductionState> {
  const active = await requireManager();
...
export async function deleteCustomDeduction(formData: FormData): Promise<void> {
  const active = await requireManager();
  if (!active) return;
...
/** Replace a group's items from the submitted statutory codes + custom type ids. */
async function writeGroupItems(
  ...
  const rows = [
    ...statutory
      .filter((c) => isStatutoryCode(c))
      .map((code) => ({ company_id: companyId, group_id: groupId, statutory_code: code, custom_type_id: null })),
    ...customIds.map((id) => ({ company_id: companyId, group_id: groupId, statutory_code: null, custom_type_id: id })),
  ];
  if (rows.length === 0) return null;

  const { error: insErr } = await db.from("deduction_group_items").insert(rows);
...
export async function createGroup(
  ...
  const itemsErr = await writeGroupItems(
    ...
    active.id,
    data.id,
    formData.getAll("statutory").map(String),
